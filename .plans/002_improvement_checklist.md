...
- [x] 17. TV / booth mode (10-foot UI). Done: header **TV** toggle (`AppSettings.tv_mode`, serde default): whole UI at `zoom: 1.25`, wider player column, **Up next** strip (next 3) under the player; `80vh` wrapper so the zoomed layout is exactly one screen. **Bug found on the way:** a stray `box-shadow …; }` since the first commit swallowed the `.ktv-split-stage { display: grid }` rule, so the intended side-by-side desktop layout (player left, songbook right, no page scroll) never applied; fixed, narrow screens explicitly keep the stacked layout. e2e: desktop grid + one screen at 1280, TV mode at 1080p (Up next, fits, remembered).
