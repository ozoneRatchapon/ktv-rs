...
- [~] Deploy: owner picked Cloudflare (2026-09-26). Assets-only Worker `ktv-rs` (`wrangler.jsonc`, no workers-rs until the phone remote), `deploy/_headers` (CSP incl. required `'unsafe-eval'` + `blob:`, mic Permissions-Policy, immutable hashed assets), `tools/build_web.sh` shared by CI + `deploy.sh` (main-only, clean, pushed; `--dry-run` anywhere). Verified via `wrangler dev`: headers, SPA fallback, playback + guide + Blob worklet with zero CSP violations; dry-run ok. [ ] First real deploy + `develop → main` release + tag `[gate: owner go]`.
