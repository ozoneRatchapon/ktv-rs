...
- **Synchronized Vocal Switch (Time-Preserved with Intro Offset Compensation):** Toggling between Karaoke and Original singer MV preserves the singing moment seamlessly without restarting from 0. Each song has a measured `GuideTrack { offset_secs, rate }` (timed by ear with the in-app Guide Timing Tools, Issue 001 updates 2026-09-26; the old yt-dlp `tools/guide_align.py` was removed); the karaoke video stays visible (muted) while a hidden guide player is rate-controlled to stay within ~0.1s.
